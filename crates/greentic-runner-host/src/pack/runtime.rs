...
use super::flows::{
    FlowDescriptor, ManifestLoad, PackFlows, flow_kind_to_str, load_manifest_and_flows,
    load_manifest_and_flows_from_dir,
};
use super::helpers::{
    HTTP_CLIENT, deserialize_json_bytes, locate_pack_assets, normalize_pack_path,
    normalize_schema_ref, path_is_gtpack, run_on_wasi_thread,
};
use super::host_state::HostState;
use super::i18n::I18nCatalog;
use super::loaders::{
    PackComponent, compile_component_with_cache, load_components_from_archive,
    load_components_from_dir, load_components_from_overrides, load_components_from_sources,
};
use super::metadata::PackMetadata;
use super::resolution::{
    ComponentResolution, component_sources_table, component_sources_table_from_pack_lock,
    component_specs, find_pack_lock_roots, load_pack_lock,
};
...
impl PackRuntime {
    ...
    pub async fn load(
        ...
        let materialized_root = component_resolution
            .materialized_root
            .clone()
        ...
        let component_sources_payload = if pack_lock.is_none() {
            manifest
                .as_ref()
                .and_then(|m| m.get_component_sources_v1().ok())
                .flatten()
        } else {
            None
        };
    ...
    pub async fn invoke_component(
        ...
        run_on_wasi_thread("component.invoke", move || {
            ...
            let result = HostState::instantiate_component_result(
                &mut linker,
                &mut store,
                &component,
                &ctx,
    ...
    pub fn load_i18n_catalog(&self, locale: &str) -> Result<Option<I18nCatalog>> {
        ...
        if let Ok(bytes) = self.read_asset(&path) {
            let json: Value = serde_json::from_slice(&bytes)
                .with_context(|| format!("parse i18n bundle: {}", path))?;
            return Ok(Some(
                I18nCatalog::from_json(locale, &json)
                    .map_err(|e| anyhow!("invalid i18n bundle {}: {}", path, e))?,
            ));
        }
        ...
        if lang != locale {
            ...
            if let Ok(bytes) = self.read_asset(&lang_path) {
                let json: Value = serde_json::from_slice(&bytes)
                    .with_context(|| format!("parse i18n bundle: {}", lang_path))?;
                return Ok(Some(I18nCatalog::from_json(lang, &json).map_err(|e| {
                    anyhow!("invalid i18n bundle {}: {}", lang_path, e)
                })?));
            }
        }
        Ok(None)
    ...
    pub async fn invoke_provider(
        ...
        run_on_wasi_thread("provider.invoke", move || {
            ...
            let result = if use_schema_core {
                ...
                bindings
                    .greentic_provider_schema_core_schema_core_api()
                    .call_invoke(&mut store, &op_owned, &input_json)?
            } else {
                let pre_instance = pre_instance
                    .take()
                    .ok_or_else(|| anyhow!("provider pre_instance already consumed"))?;
                ...
                let bindings = block_on(async { pre.instantiate_async(&mut store).await })?;
                bindings
                    .greentic_provider_core_schema_core_api()
                    .call_invoke(&mut store, &op_owned, &input_json)?
            };
    ...
    pub fn describe_component_contract_v0_6(&self, component_ref: &str) -> Result<Option<Value>> {
        ...
        run_on_wasi_thread("component.describe", move || {
            ...
            let pre = match crate::component_api::v0_6_descriptor::ComponentV0V6V0Pre::new(
                pre_instance,
            ) {
                Ok(pre) => pre,
