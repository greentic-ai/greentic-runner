//! Gated NATS round-trip integration test for `telco-x-event-bridge`.
//!
...
//!   cargo test -p telco-x-event-bridge --test nats_roundtrip
//! ```
...
use serde_json::json;
use telco_x_event_bridge::{EchoInvoker, run_bridge};
use tokio::time::{Duration, timeout};

const REQUEST_SUBJECT: &str = "greentic.telco-x.request.v1";
const RESPONSE_SUBJECT: &str = "greentic.telco-x.response.v1";
const NATS_URL_ENV: &str = "GREENTIC_TEST_NATS_URL";

#[tokio::test(flavor = "multi_thread")]
async fn nats_roundtrip_telco_x_request_response() {
    ...
    let bridge_handle = tokio::spawn(run_bridge(bridge_client, Arc::new(EchoInvoker)));
    ...
    assert_eq!(response.output["echo"], json!(true));
    assert_eq!(response.output["target"], json!("line-1"));
    assert_eq!(response.output["operation"], json!("provision"));
